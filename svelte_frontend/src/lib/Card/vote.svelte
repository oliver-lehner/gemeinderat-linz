...
{#if vote}
	...
	>
		<div class="{xs ? 'h-16 w-16' : 'h-24 w-24 sm:w-36 sm:h-36'} border-2 row-span-2 border-white shadow-md relative justify-self-center sm:justify-self-start">
			<PieChart {vote} {submitter} />
			<div class="absolute inset-0 z-10 bg-transparent shadow-inner" />
		</div>
		<h3 class="line-clamp-2 col-span-2 leading-5 h-10 ml-2 mb-1 overflow-hidden {xs? 'text-xs' : ''}">{vote.subject}</h3>
		{#each vote.meta as metaInfo}
			<div
				class="{metaInfo.includes('angenommen') || metaInfo.includes('zugewiesen')
					...
					? 'bg-red-500'
					: 'bg-gray-500'} w-full rounded sm:ml-0 p-2 {xs?'col-start-1 col-span-3' : 'col-start-2 col-span-2 ml-2'} sm:col-start-1 shadow-md sm:mt-2 order-2 "
			>
